...
const OUT = process.env.OUT || '.ref/out/png/preview';
...
(async () => {
  ...
  /* ⚠ 试过的坑：
       ① clip.scale 与 deviceScaleFactor **相乘** → 936dpi；
       ② 把 clip 的 x/y/w/h 预先除以 DSF → 只截到页面左下 1/3.125，
          输出还是 794×1122（等于没缩放），横线位置全乱。
     正解：clip 用**原样的 CSS 坐标**、scale 留 1，缩放交给 deviceScaleFactor。
     另外 `.as-page` 只是白纸本身，但外壳有深色底 —— 先把页面搬进一个
     白底 fixed 容器（见上一步），否则每行会多出 3000+ 深像素。 */
  const DSF = 300 / 96;
