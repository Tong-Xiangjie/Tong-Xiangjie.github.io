...
(async () => {
  ...
  const E = `(() => {
    ...
    /* 直接调分页器，看它自己算出的 face */
    const C = window.AS.config, G = window.AS.geometry, S = window.AS.subject;
    const p = C.PRESETS.A4;
    const blocks = Pg.planBlocks({
      preset: p, group: C.GROUPS.A4, faceW: 210,
      choiceTotal: 12, choiceStart: 1,
      subjItems: [
    ...
    const faces = Pg.paginate({ blocks: blocks, preset: p,
      chromeFirst: 82.95, chromeOther: 0 });
    return {
      hasSubjBoxBottom: 'subjBoxBottom' in (faces[0] || {}),
      faces0: faces.map(f => ({ first: f.first, blank: f.blank,
        subjBoxBottom: f.subjBoxBottom, cursor: f.cursor,
        body: f.body.map(b => b.kind === 'choice'
          ? { k: 'c', boxTop: b.boxTop, rows: b.rows }
          : { k: 's', no: b.item.no, boxTop: b.boxTop, h: b.h }) })),
      pagSrc: Pg.paginate.toString().slice(0, 60),
      subjItemFix: Pg.SUBJ_ITEM_FIX,
      scripts: src
    };
  })()`;
