...
const PAGE = process.env.PAGE || 'fonttest3.html';
const OUTPDF = process.env.OUTPDF || 'fonttest3.pdf';
const CHROME = 'C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe';
const PORT = Number(process.env.PORT || 9639);
...
(async () => {
  ...
  await send('Page.navigate', { url: `http://127.0.0.1:8137/.ref/${PAGE}?cb=` + Date.now() }, 25000);
  await sleep(2500);
  const r = await send('Runtime.evaluate', { expression:
    `(async () => await window.runTest())()`, returnByValue: true, awaitPromise: true }, 90000);
  if (r.__to) say('EVAL TIMEOUT');
  const v = r.result?.result?.value;
  say('RESULT =', JSON.stringify(v ? Object.assign({}, v, { uri: undefined }) : r.result?.exceptionDetails?.exception, null, 1));
  if (v && v.uri) {
