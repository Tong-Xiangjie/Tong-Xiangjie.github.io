...
const PORT = Number(process.env.PORT || 9651);
...
(async () => {
  ...
  await send('Page.navigate', { url: 'http://127.0.0.1:8137/.ref/rectdiag.html?cb=' + Date.now() }, 30000);
  let rdy = false;
  for (let i = 0; i < 60 && !rdy; i++) {
    await sleep(400);
    const q = await send('Runtime.evaluate', { expression: 'typeof window.runDiag', returnByValue: true }, 10000);
    rdy = q.result?.result?.value === 'function';
  }
  say('runDiag ready =', rdy);
  if (!rdy) { clearTimeout(HARD); try { chrome.kill(); } catch {} process.exit(4); }
  const expr = `(async () => { try { return await window.runDiag(${JSON.stringify(CFG)}); }
