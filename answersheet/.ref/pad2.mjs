/* 量黑框内上/下空档（用 dbg_mm 的可靠算法：直接由 rect 求差）。
   用法：node .ref\pad2.mjs [A4|A3] [题数] */
...
async function main() {
  ...
  const EXPR = `(() => {
    ...
    let maxBottom = -1e9;
    last.querySelectorAll('.as-bubble').forEach(b => {
      const v = r(b).bottom; if (v > maxBottom) maxBottom = v;
    });
    /* 首行题号块的上缘 */
    const firstNum = lines[0].querySelector('.as-choice-num');
    const ir = r(inner);
    return {
      padTop: ((r(firstNum).top - (ir.top + bw)) * MM),
      padBottom: (((ir.bottom - bw) - maxBottom) * MM),
      cssTop: parseFloat(cs.paddingTop) * MM,
      cssBottom: parseFloat(cs.paddingBottom) * MM,
      nLines: lines.length
    };
  })()`;
  ...
  if (res.result?.exceptionDetails) {
    ...
    console.log('  CSS padding  top %s  bottom %s', f(d.cssTop), f(d.cssBottom));
    console.log('  ▶ 题号行上缘 → 黑框上内缘 = %s mm   (目标 3)', f(d.padTop));
    console.log('  ▶ 末行选项下缘 → 黑框下内缘 = %s mm   (目标 3)', f(d.padBottom));
  }
