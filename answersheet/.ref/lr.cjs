...
(async () => {
  ...
  const E = `(() => {
    ...
    const cb = blocks[0];
    const lr = cb.lineRanges;
    /* 手工复算 endNo，看是哪一步出问题 */
    const choiceLine = 8, rows = 7;
    const idx = choiceLine + rows - 1;
    return {
      lrLen: lr.length,
      lr0: lr[0], lr8: lr[8], lr13: lr[13], lr14: lr[14],
      idx: idx, atIdx: lr[idx],
      atIdxTo: lr[idx] ? lr[idx].to : 'UNDEFINED',
      rowsField: cb.rows, totalLines: cb.totalLines,
      firstBody: null
    };
  })()`;
