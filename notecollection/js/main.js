// ========== 强制所有链接在当前页面打开（自动转换 target="_blank" 为 _self）==========
(function() {
    // 监听整个页面，动态转换所有链接
    function convertAllLinks() {
        document.querySelectorAll('a[href]').forEach(function(link) {
            ...
            // 移除 rel="noopener noreferrer"（如果有的话，避免干扰）
...
// 然后删除原来的 history.scrollRestoration = 'manual';
// 替换为滚动保存恢复代码：

const SCROLL_KEY = 'banknote_scroll_pos';
...
// 页面加载时判断
const fromOutside = document.referrer && !document.referrer.includes(location.host);

if (fromOutside && sessionStorage.getItem(SCROLL_KEY)) {
    // 从外部网站返回 → 恢复滚动位置
    const pos = parseInt(sessionStorage.getItem(SCROLL_KEY));
