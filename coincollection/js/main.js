            ...
            if (series.varieties && series.varieties.length > 0) {
                for (let vi = 0; vi < series.varieties.length; vi++) {
                    ...
                    for (let ci = 0; ci < variety.copies.length; ci++) {
                        ...
                        if (isEmptySearch) {
                            ...
                            switch(type) {
                                case 'all':
                                    const searchText = `${series.seriesName} ${variety.varietyName} ${copy.country || ''} ${copy.year} ${copy.material || ''} ${copy.grade || ''} ${copy.gradingCompany || ''}`.toLowerCase();
                                ...
                                case 'company':
                                    ...
                                    break;
                                default:
                                    const defaultText = `${series.seriesName} ${variety.varietyName} ${copy.grade || ''} ${copy.year} ${copy.material || ''} ${copy.country || ''}`.toLowerCase();
                ...
                for (let ci = 0; ci < series.copies.length; ci++) {
                    ...
                    if (isEmptySearch) {
                        ...
                        switch(type) {
                            case 'all':
                                const searchText = `${series.seriesName} ${copy.country || ''} ${copy.year} ${copy.material || ''} ${copy.grade || ''} ${copy.gradingCompany || ''}`.toLowerCase();
                                match = searchText.includes(lowerKeyword);
                                break;
                            case 'name':
    ...
    for (let cid of categoryOrder) {
        ...
        html += `
            <div class="search-category-divider">
                ...
                <span class="category-count">${groupResults.length}枚</span>
            </div>
        `;
    ...
    for (let idx = 0; idx < cat.series.length; idx++) {
        ...
        items += `
            <div class="series-item" onclick="selectSeriesOrVariety('${cid}', ${idx})">
                ...
                <div class="series-count">${totalCount}枚</div>
    ...
    for (let vi = 0; vi < varieties.length; vi++) {
        ...
        itemsHtml += `
            <div class="series-item" onclick="selectVariety('${cid}', ${si}, ${vi})">
                ...
                <div class="series-count">${copyCount}枚</div>
