...
点击藏品图片（**文章阅读器里的配图也能点**）打开全屏查看弹窗，支持：

- **双指捏合缩放 / 桌面滚轮缩放**（1~8 倍，触摸与桌面共用同一上限；滚轮以光标为锚点，Ctrl+滚轮不抢，留给浏览器自身的页面缩放）
- **拖拽平移**（放大后；拖动结束不会误触发"单击关闭"）
- **正反面切换**：弹窗左右两侧的圆形 **‹ ›** 按钮，或键盘 **← →**。正反面是同一张图时（文章配图、专题灯箱就是这么调用的）按钮自动不显示
- **单击关闭**（鼠标与触摸一致，点哪儿都关，含图片本身；右上角 × 与 **Esc** 同样可用）
  - 翻面按钮与关闭按钮长在蒙版里，它们的点击会被"点哪儿都关"放过，不会顺手关掉弹窗
  - **Esc 是分层的**：弹窗开在详细信息卡片 / 专题灯箱上面时，Esc 只关弹窗那一层（键盘事件抢先注册并 `stopImmediatePropagation`，专题灯箱也用 ← → 切换景观，不拦就会同时翻两处）
- 缩回 1 倍用滚轮向下 / 双指捏合（**没有"双击还原"** —— 与"点哪儿都关"互斥：双击的第一下就已经关了弹窗）
- **原图没解码完之前禁止缩放与拖动**：那时画面还只是低清垫底图，拖它没有意义（原图盖上来位置还会错位）。此时底部提示语会变成「原图加载中…」，由 `modalFullReady` 统一把关（Hammer 的 pinch/pan 与滚轮都检查它）
...
- 来源本身就是**已加载完的原图**时（文章配图走这条）连垫底图都不用，顺带省掉一次注定 404 的缩略图请求（文章配图没有生成缩略图）
...
- **配图可点开看大图** — 文章正文里的图片，点一下就用同一个图片查看器全屏看（文档级事件委托，不依赖每次渲染的时机）
...
**明暗（三态）**：「我的 → 外观」，可选 **跟随系统 / 亮 / 暗**，存 localStorage 的 `collection-color-scheme`。

- `theme.js` 的 `isDarkScheme()` 解析三态（`system` 时读 `matchMedia('(prefers-color-scheme: dark)')`，并监听系统变化实时跟随）
- ⚠️ **暗色必须走 JS，不能只写 CSS**：整套配色是 `applyTheme()` 用**内联样式**写到 `documentElement` 上的，内联优先级高于任何样式表规则 —— 纯 CSS 覆盖会被压掉。所以 `layout.css` 里那份 `html[data-color-scheme="dark"]` 只负责"**JS 跑起来之前的临时底色**"（防暗色下先闪一下白屏），真正的暗色调色板由 `applyTheme()` 按主题色**往黑里压**算出来（`darkenColor()`），保持"整套配色都带主题色倾向"的既有观感
- `index.html` 的 `<head>` 里有一小段内联脚本，在**任何样式/绘制之前**就打上 `data-color-scheme` 与 `color-scheme`，并提前套上保存的主题色（原来主题色要等 `DOMContentLoaded` 才生效，会先闪一下默认蓝）
- **强调色 `--theme` 在暗色下刻意不变**：提亮它会让「主题色实心按钮 + 白字」的对比度掉到 WCAG 以下（#1677ff→#4096ff 时对比度从 4.6:1 掉到约 2.9:1）。代价是很暗的预设（墨绿 #237804、靛蓝 #2f54eb）作为**文字色**时在暗底上偏暗，介意就换个亮一点的预设
- `color-scheme` 也跟着设，原生控件与滚动条才会跟着变暗

**主题色**：

- 预设 11 种主题色（按色相排列：蓝 / 靛蓝 / 紫 / 玫红 / 红 / 铜 / 金 / 橙 / 绿 / 墨绿 / 青碧；其中「铜」呼应站名）
- 支持**自定义颜色**，使用取色器选取后添加，**不限制个数**
- 主题色自动调整边框、背景、侧边栏等关联颜色（`applyTheme()` 里用 `lightenColor()` 派生；暗色下改用 `darkenColor()`）

### 9. 侧边栏折叠

点击左上角按钮可折叠/展开侧边栏。

**折叠状态是两个板块共用的**（`toggleSidebar()` 在 `core.js` 里一次写入 `modeStates.notes` 与
`modeStates.coins`）——这是有意为之：折叠是"我想让内容区更宽"的界面偏好，不随板块切换而变，
避免在纸币折叠、切到硬币又自动展开的跳变。

> 修正记录：本节此前写的是"状态在纸币和硬币板块之间独立保存"，与实现不符。
> 实现从最初就是共用的，属于文档写错，已按实际行为更正（代码未改）。
...
| 文章 | 文章阅读、全文搜索、文章缓存重载 |
| 我的 | 统计概览、价格列表、评级/年代统计、外观（明暗）、主题色、网格画质、图片缓存、文章缓存、离线预缓存、数据导出 |
