...
- **共享元素过渡** — 点开大图从被点击的缩略图位置「生长」到全屏，关闭时缩回；用独立飞行图层（`.modal-flight`）实现，**完全不触碰 Hammer 的捏合缩放**。缩略图是原图等比缩放，故缩略图矩形与全屏 contain 矩形宽高比一致，只需 `translate` + 均匀 `scale` —— 无畸变且全程 GPU 合成；关闭时若缩略图已滚出视口则直接关闭
